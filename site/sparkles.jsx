const { useEffect, useState, useId, useRef } = window.React;
const { motion, animate, useMotionValue } = window; // Framer motion exposes window.motion
...
// Sparkles Component using vanilla tsParticles to avoid React wrapper CDN issues
function Sparkles({
  className,
  size = 1,
  minSize = null,
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      window.tsParticles.load({ id, options: { ...defaultOptions, ...options } });
    };
    init();
    
    return () => {
      const p = window.tsParticles.particles().find(p => p.id === id);
      if (p) p.destroy();
    };
  }, [id, size, minSize, density, speed, minSpeed, opacity, opacitySpeed, minOpacity, color, background, options]);
...
// Logos
const Retool = () => (
  <svg viewBox="0 0 180 56" fill="currentColor" className="w-full h-full opacity-60 hover:opacity-100 transition-opacity">
...
const Vercel = () => (
  <svg viewBox="0 0 180 54" fill="currentColor" className="w-full h-full opacity-60 hover:opacity-100 transition-opacity">
...
const logos = [
  { id: "retool", component: Retool, className: "w-32 h-12 text-[#9ca3af]" },
  { id: "vercel", component: Vercel, className: "w-32 h-12 text-[#9ca3af]" },
  { id: "remote", component: Remote, className: "w-32 h-12 text-[#9ca3af]" },
  { id: "arc", component: Arc, className: "w-32 h-12 text-[#9ca3af]" },
  { id: "raycast", component: Raycast, className: "w-32 h-12 text-[#9ca3af]" }
];

function PartnersSection({ t }) {
  // Uses dark theme styling since it comes after DarkSection
  return (
    <section id="partenaires" style={{ background: 'var(--navy-deep)', padding: '60px 0 120px', position: 'relative' }}>
      <div className="w-full overflow-hidden flex flex-col items-center">
        <div className="mx-auto w-full max-w-4xl px-8">
          <div className="text-center text-3xl font-bold" style={{ color: '#fff', fontFamily: 'var(--font-heading)' }}>
            <span style={{ color: 'var(--accent)' }}>
              La confiance de nos partenaires.
            </span>
            <br />
            <span>Des équipements à la pointe pour vous.</span>
          </div>

          <div className="relative mt-12 h-[80px] w-full">
            <InfiniteSlider 
            ...
            >
              {logos.map(({ id, component: Logo, className }) => (
                <div key={id} className={className} style={{ display: 'flex', alignItems: 'center' }}>
                  <Logo />
                </div>
              ))}
            </InfiniteSlider>
            
            {/* Left progressive blur overlay for dark bg */}
            <div className='pointer-events-none absolute top-0 left-0 h-full w-[150px]' style={{
              background: `linear-gradient(to right, var(--navy-deep) 0%, transparent 100%)`
            }} />
            
            {/* Right progressive blur overlay for dark bg */}
            <div className='pointer-events-none absolute top-0 right-0 h-full w-[150px]' style={{
              background: `linear-gradient(to left, var(--navy-deep) 0%, transparent 100%)`
            }} />
          </div>
        </div>

        <div className="relative mt-12 h-64 w-full overflow-hidden [mask-image:radial-gradient(50%_50%,white,transparent)]" style={{ WebkitMaskImage: 'radial-gradient(50% 50%, white, transparent)' }}>
          <div className="absolute inset-0 before:absolute before:inset-0 before:opacity-40" style={{ '--tw-bg-opacity': 0.4, background: `radial-gradient(circle at bottom center, ${t.accent}, transparent 70%)` }} />
          <div className="absolute -left-1/2 top-1/2 aspect-[1/0.7] z-10 w-[200%] rounded-[100%] border-t" style={{ borderColor: 'rgba(255,255,255,0.1)', background: 'var(--navy-deep)' }} />
          <Sparkles
            density={800}
            className="absolute inset-x-0 bottom-0 h-full w-full"
            style={{ maskImage: 'radial-gradient(50% 50%, white, transparent 85%)', WebkitMaskImage: 'radial-gradient(50% 50%, white, transparent 85%)' }}
            color="#ffffff"
          />
        </div>
      </div>
